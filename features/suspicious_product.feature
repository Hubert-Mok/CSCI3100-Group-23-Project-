Feature: Suspicious product moderation
  ...
  Scenario: Suspicious title is flagged and hidden from marketplace index
    Given a verified seller exists with email "seller_title@link.cuhk.edu.hk" and password "Password123"
    When the seller creates a suspicious product listing with title "suspicious laptop, contact via whatsapp 12345678" and description "Clean description for a normal listing"
    Then the listing should be flagged for moderation
    And the listing status should be "pending"
    And the flagged listing should not appear on the marketplace index
